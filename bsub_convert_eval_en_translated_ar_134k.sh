#!/bin/bash
# Convert en_translated_ar step-133600 (134k run) to HF format, then submit all evals.
#
# Submit with:
#   source /etc/profile && cd /gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual
#   bsub -G grp_exploratory -q normal -J convert_eval_en_translated_ar_134k \
#     -n 32 -R "span[ptile=32]" -R "rusage[mem=200G]" \
#     -cwd /gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual \
#     -o logs/%J_convert_eval_en_translated_ar_134k.out \
#     -e logs/%J_convert_eval_en_translated_ar_134k.err -env "all" \
#     bash /gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual/bsub_convert_eval_en_translated_ar_134k.sh

set -euo pipefail

PROJ=/gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual
CKPT=$PROJ/outputs/.outputs/llama3_7B_test2_en_translated_ar_stage1_134k_clean_injection_0_20_100_1000_20800entities_seq2048/step-133600
...
OUT_HF=$PROJ/outputs/hf_export/llama3_7b_en_translated_ar_134k
EVAL_OUT=$PROJ/outputs/lm_eval_results/global_evals_en_translated_ar_134k

source $PROJ/.venv/bin/activate

echo "=== Converting en_translated_ar 134k step-133600 to HF format ==="
python $CONVERT \
    $CKPT $OUT_HF \
    --model_name llama3 \
