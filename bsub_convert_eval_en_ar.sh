#!/bin/bash
# Convert en_ar step-33000 checkpoint to HF format, then submit all evals.
#
# Submit with:
#   source /etc/profile && cd /gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual
#   bsub -G grp_exploratory -q normal -J convert_eval_en_ar \
#     -n 32 -R "span[ptile=32]" -R "rusage[mem=200G]" \
#     -cwd /gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual \
#     -o logs/%J_convert_eval_en_ar.out -e logs/%J_convert_eval_en_ar.err -env "all" \
#     bash /gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual/bsub_convert_eval_en_ar.sh

set -euo pipefail

PROJ=/gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual
CKPT=$PROJ/outputs/.outputs/llama3_7B_test3_en_ar_stage1_34k_clean_injection_0_20_100_1000_20800entities_seq2048/step-33000
...
OUT_HF=$PROJ/outputs/hf_export/llama3_7b_en_ar_step33000
EVAL_OUT=$PROJ/outputs/lm_eval_results/global_evals_en_ar

source $PROJ/.venv/bin/activate

echo "=== Converting en_ar step-33000 to HF format ==="
python $CONVERT \
    $CKPT $OUT_HF \
    --model_name llama3 \
...
mkdir -p "$EVAL_OUT"
bash $PROJ/eval_scripts/bsub_global_eval.sh "$OUT_HF" "$EVAL_OUT" "en_ar"
...
"
echo "Submitted fictive entity 2ratemix eval for en_ar"

echo "=== All jobs submitted for en_ar. HF model: $OUT_HF ==="
