#!/bin/bash
# Convert en_anchored_ar step-133600 to HF format, then submit all evals.
# Tokenizer: 65k_paired (Arabic+English paired vocab)
# Model flavor: 7B_flex (single vocab)
#
# Submit with:
#   source /etc/profile && cd /gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual
#   bsub -G grp_exploratory -q normal -J convert_eval_en_anchored_ar \
#     -n 32 -R "span[ptile=32]" -R "rusage[mem=200G]" \
#     -cwd /gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual \
#     -o logs/%J_convert_eval_en_anchored_ar.out \
#     -e logs/%J_convert_eval_en_anchored_ar.err -env "all" \
#     bash /gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual/bsub_convert_eval_en_anchored_ar.sh

set -euo pipefail

PROJ=/gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual
CKPT=$PROJ/outputs/.outputs/llama3_7B_test6_en_anchored_ar_stage6_34k_clean_injection_0_20_100_1000_20800entities_seq2048/step-133600
...
OUT_HF=$PROJ/outputs/hf_export/llama3_7b_en_anchored_ar
EVAL_OUT=$PROJ/outputs/lm_eval_results/global_evals_en_anchored_ar

source $PROJ/.venv/bin/activate

echo "=== Converting en_anchored_ar step-133600 to HF format ==="
python $CONVERT \
    $CKPT $OUT_HF \
    --model_name llama3 \
...
echo "=== Submitting global PIQA (IQA) eval ==="
bsub -G grp_exploratory -q normal -n 4 \
  -R "rusage[mem=64000] span[hosts=1]" \
  -gpu "num=1:mode=exclusive_process" \
