#!/bin/bash
# Convert en_ru step-133600 to HF format, then submit all evals.
# Tokenizer: 65k_en1.0_ru1.0 (Russian+English paired vocab)
# Model flavor: 7B_flex (single vocab, NOT 2xvocab)
#
# Submit with:
#   source /etc/profile && cd /gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual
#   bsub -G grp_exploratory -q normal -J convert_eval_en_ru \
#     -n 32 -R "span[ptile=32]" -R "rusage[mem=200G]" \
#     -cwd /gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual \
#     -o logs/%J_convert_eval_en_ru.out \
#     -e logs/%J_convert_eval_en_ru.err -env "all" \
#     bash /gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual/bsub_convert_eval_en_ru.sh

set -euo pipefail

PROJ=/gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual
CKPT=$PROJ/outputs/.outputs/llama3_7B_test4_en_ru_stage1_34k_clean_injection_0_20_100_1000_20800entities_seq2048/step-133600
...
OUT_HF=$PROJ/outputs/hf_export/llama3_7b_en_ru
EVAL_OUT=$PROJ/outputs/lm_eval_results/global_evals_en_ru

source $PROJ/.venv/bin/activate

echo "=== Converting en_ru step-133600 to HF format ==="
python $CONVERT \
    $CKPT $OUT_HF \
    --model_name llama3 \
