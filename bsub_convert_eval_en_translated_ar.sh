#!/bin/bash
# Convert en_translated_ar step-33400 checkpoint to HF format, then submit all evals.
#
# Submit with:
#   source /etc/profile && cd /gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual
#   bsub -G grp_exploratory -q normal -J convert_eval_en_translated_ar \
#     -n 32 -R "span[ptile=32]" -R "rusage[mem=200G]" \
#     -cwd /gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual \
#     -o logs/%J_convert_eval_en_translated_ar.out -e logs/%J_convert_eval_en_translated_ar.err -env "all" \
#     bash /gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual/bsub_convert_eval_en_translated_ar.sh

set -euo pipefail

PROJ=/gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual
CKPT=$PROJ/outputs/.outputs/llama3_7B_test2_en_translated_ar_stage1_34k_clean_injection_0_20_100_1000_20800entities_seq2048/step-33400
...
OUT_HF=$PROJ/outputs/hf_export/llama3_7b_en_translated_ar
EVAL_OUT=$PROJ/outputs/lm_eval_results/global_evals_en_translated_ar

source $PROJ/.venv/bin/activate

echo "=== Converting en_translated_ar step-33400 to HF format ==="
python $CONVERT \
    $CKPT $OUT_HF \
    --model_name llama3 \
...
echo "=== Submitting global evals (mmlu EN/AR + piqa) ==="
mkdir -p "$EVAL_OUT"
bash $PROJ/eval_scripts/bsub_global_eval.sh "$OUT_HF" "$EVAL_OUT" "en_translated_ar"

echo "=== Submitting fictional entity eval (2ratemix) ==="
bsub -G grp_exploratory \
  -q normal -n 4 \
  -R "rusage[mem=64000] span[hosts=1]" \
  -gpu "num=1:mode=exclusive_process" \
  -o "$EVAL_OUT/fictive_entity_2ratemix_%J.log" \
  -J "fictive_en_translated_ar" \
  bash -c "
source $PROJ/.venv/bin/activate
export HF_DATASETS_CACHE=$EVAL_OUT/.hf_cache_fictive_\$\$
python -m lm_eval \
  --model hf \
  --model_args pretrained=$OUT_HF,dtype=bfloat16 \
  --tasks fictive_entity_all_rates \
  --output_path $EVAL_OUT/fictive_entity_2ratemix \
  --batch_size 8 \
  --include_path $LM_HARNESS/custom_evals/gemini_seeds_en_2ratemix_fictive_entity_eval_suite
"
echo "Submitted fictive entity 2ratemix eval for en_translated_ar"

echo "=== All jobs submitted for en_translated_ar. HF model: $OUT_HF ==="
echo "=== Eval results will appear in: $EVAL_OUT ==="
