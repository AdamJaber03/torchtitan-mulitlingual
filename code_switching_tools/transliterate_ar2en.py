...
# This will catch Chinese, Russian, Emojis, and stray Arabic punctuation.
NON_ENGLISH_PATTERN = re.compile(r'[^\x00-\x7F\u060C\u061B\u061F]+')

def transliterate_match(match):
    """Takes an Arabic regex match and forces it into English characters."""
    ...
    with open(filepath, 'rb') as f_in, open(out_filepath, 'wb') as f_out:
        for line in f_in:
            ...
            try:
                ...
                continue # Gracefully skip the corrupted lines we discussed
                
            text = doc.get("text", "")
            if not text: continue
            
            # STEP 1: Transliterate any remaining Arabic words
            cleaned_text = AR_PATTERN.sub(transliterate_match, text)
                        
            # STEP 1.5: Convert Arabic punctuation to English equivalents
