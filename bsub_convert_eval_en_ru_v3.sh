#!/bin/bash
# Convert en_ru step-133600 (v3: correct en/ru tokenizer) to HF format, then submit all evals.
#
# Submit with:
#   source /etc/profile && cd /gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual
#   bsub -G grp_exploratory -q normal -J convert_eval_en_ru_v3 \
#     -n 32 -R "span[ptile=32]" -R "rusage[mem=200G]" \
#     -cwd /gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual \
#     -o logs/%J_convert_eval_en_ru_v3.out \
#     -e logs/%J_convert_eval_en_ru_v3.err -env "all" \
#     bash /gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual/bsub_convert_eval_en_ru_v3.sh

set -euo pipefail

PROJ=/gpfs/ess6000-1/proj/dmfexp/trAr/torchtitan-mulitlingual
CKPT=$PROJ/outputs/.outputs/llama3_7B_test4_en_ru_v3_ru_tokenizer_injection_0_20_100_1000_20800entities_seq2048/step-133600
...
OUT_HF=$PROJ/outputs/hf_export/llama3_7b_en_ru_v3
EVAL_OUT=$PROJ/outputs/lm_eval_results/global_evals_en_ru_v3

source $PROJ/.venv/bin/activate

echo "=== Converting en_ru step-133600 (v3) to HF format ==="
python $CONVERT \
    $CKPT $OUT_HF \
    --model_name llama3 \
